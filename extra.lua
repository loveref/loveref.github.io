...
return function (api)
    ...
    local function doDescriptions(t)
            ...
            t.minidescriptiont = t.minidescriptiont or {}
            for language, translation in pairs(t.descriptiont) do
                    t.minidescriptiont[language] = t.minidescriptiont[language] or translation:match('[^\n]+')
            end
    end
    ...
    for _, module_ in ipairs(api.modules) do
        for _, type_ in ipairs(module_.types) do
            ...
            for _, subtype in ipairs(type_.subtypes) do
                local s = typesByName[subtype]
                if s then
                    table.insert(subtypes, s)
                end
            end
